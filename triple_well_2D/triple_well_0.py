...
#%%
#states
initial_state = np.array([-5.7717539205491332765518563974183052778244018554687500000000000000000000e+00,  5.1317952947406419965231315746996294119286474710861511994153261184692383e-13])
saddle_state = np.array([0, -2.93429453258859905540045787120106979273259639739990234375e-02])
#saddle_state = np.array([0, 18])
target_state = np.array([5.7717539205491332765518563974183052778244018554687500000000000000000000e+00, 5.1317952947406419965231315746996294119286474710861511994153261184692383e-13])
...
if potential_well_plot:
    ...
    xx, yy = np.meshgrid(x, y)
    pot = potential(xx,yy)
    im = plt.contourf(xx, yy, pot, 30, cmap = 'RdBu_r')
    #plt.contour(xx, yy, pot, 30)
    plt.xlabel('x')
    plt.ylabel('y')
    plt.grid()
    cbar = fig.colorbar(im, format = '%.0f')
    plt.scatter(initial_state[0],initial_state[1], marker = 'o', label = '$X_A$', color = 'black', s=40)
    plt.scatter(target_state[0], target_state[1], marker = 'x', label = '$X_B$', color = 'black', s=40)
