...
while k<kmax and incr > threshold:
    ...
    if plot:
        ...
        if k==1:
            plt.figure(1)
            plt.plot(phi_0[0], phi_0[1], label = 'initial')
        ...
        if k==plot_index:
            plt.figure(1)
            plt.plot(phi_i[0], phi_i[1], label = f'{plot_index} after interp')
            plt.legend()
        
        if k%freq==0:
            plt.figure(3)
            plt.plot(phi_i[0], phi_i[1], label = f'{k}')
            plt.title('all')
            plt.legend()
    k+=1
